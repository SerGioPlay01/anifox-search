/**
 * AniFox Anti-Adblock Banner 3.0
...
(() => {
  ...
  /* ---------- 4 теста на блокировщик (параллельно) ---------- */
  async function detectAdblockFast() {
    ...
    return results.filter(Boolean).length >= 2; // ≥2 положительных = блокировщик есть
  }
  ...
  function baitTest() {
    return new Promise(res => {
      ...
      el.className = 'ads ad-unit ad-banner advertisement';
      el.style.cssText = 'position:absolute;left:-9999px;top:-9999px;width:1px;height:1px;';
      el.innerHTML = '<div class="ad-text">Advertisement</div>';
  ...
  function buildBigBanner() {
    if (document.querySelector('.ab-banner')) return;
    ...
    modal.innerHTML = `
      <div class="ab-instructions-content">
        ...
        <div class="ab-instructions-grid">
          <div class="ab-instruction-item"><h4>AdBlock / AdBlock Plus</h4><ol><li>Нажмите на иконку AdBlock в браузере</li><li>Выберите "Не выполнять на страницах этого сайта"</li><li>Обновите страницу</li></ol></div>
          <div class="ab-instruction-item"><h4>uBlock Origin</h4><ol><li>Нажмите на иконку uBlock</li><li>Кликните на большую кнопку питания</li><li>Обновите страницу</li></ol></div>
          <div class="ab-instruction-item"><h4>AdGuard</h4><ol><li>Нажмите на иконку AdGuard</li><li>Выключите защиту для этого сайта</li><li>Обновите страницу</li></ol></div>
          ${isBrave ? `<div class="ab-instruction-item brave-block"><h4>Brave Browser</h4><ol><li>Нажмите на иконку льва в адресной строке</li><li>Отключите "Блокировку рекламы" для этого сайта</li><li>Обновите страницу</li></ol></div>` : ''}
          ${isGhost ? `<div class="ab-instruction-item ghostery-block"><h4>Ghostery</h4><ol><li>Нажмите на иконку призрака</li><li>Нажмите "Доверять сайту" или отключите "Блокировку рекламы"</li><li>Обновите страницу</li></ol></div>` : ''}
        </div>
        <button class="ab-btn ab-btn--main" onclick="this.closest('.ab-instructions-modal').remove(); location.reload();"><i class="fas fa-sync"></i> Обновить страницу</button>
      </div>`;
  ...
  async function run() {
    ...
    // Если ранее нажали «С блокировщиком», но щас реклама разблокирована
    ...
    // Если не выбирали ранее и реклама блокируется
    if (!choice && blocked) {
      buildBigBanner();
      return;
    }

    // Если выбрали «С блокировщиком» и реклама всё ещё блокируется
    if (choice === 'with-adblock' && blocked) {
      insertMiniBanner();
      return;
    }

    // Иначе считаем, что всё ок
    removeMiniBanner();
  }
