...
async function dbAdd(s, d)   { const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readwrite'); t.objectStore(s).add(d); t.oncomplete = () => resolve(); t.onerror = () => reject(t.error); }); }
async function dbPut(s, d)   { const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readwrite'); t.objectStore(s).put(d); t.oncomplete = () => resolve(); t.onerror = () => reject(t.error); }); }
async function dbGet(s, k)   { const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readonly'); const req = t.objectStore(s).get(k); req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); }); }
async function dbGetAll(s, i){ const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readonly'); const store = i ? t.objectStore(s).index(i) : t.objectStore(s); const req = store.getAll(); req.onsuccess = () => resolve(req.result); req.onerror = () => reject(req.error); }); }
async function dbDel(s, k)   { const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readwrite'); t.objectStore(s).delete(k); t.oncomplete = () => resolve(); t.onerror = () => reject(t.error); }); }
async function dbClear(s)    { const db = await initDB(); return new Promise((resolve, reject) => { const t = db.transaction([s], 'readwrite'); t.objectStore(s).clear(); t.oncomplete = () => resolve(); t.onerror = () => reject(t.error); }); }
...
function updateSEOMeta(apiData, rawQuery = '') {
  clearOldDynamicMeta();
  const results = (apiData && apiData.results) || [];

  // 1️⃣ Запрос из переданного rawQuery или из ЧПУ-пути
  const query = rawQuery || decodeURIComponent(location.pathname)
                               .replace(/^\/search\//, '')
                               .replace(/-/g, ' ')
                               .trim();

  // 2️⃣ Базовые значения из верстки
  ...
  // 3️⃣ Формируем SEO-контент, если есть запрос
  if (query) {
    const top = results[0];
    if (top) {
