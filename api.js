...
async function dbAdd(s, d)   { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readwrite"); t.objectStore(s).add(d); t.oncomplete = () => res(); t.onerror = () => rej(t.error); }); }
async function dbPut(s, d)   { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readwrite"); t.objectStore(s).put(d); t.oncomplete = () => res(); t.onerror = () => rej(t.error); }); }
async function dbGet(s, k)   { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readonly");  const req = t.objectStore(s).get(k); req.onsuccess = () => res(req.result); req.onerror = () => rej(req.error); }); }
async function dbGetAll(s,i) { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readonly");  const store = i ? t.objectStore(s).index(i) : t.objectStore(s); const req = store.getAll(); req.onsuccess = () => res(req.result); req.onerror = () => rej(req.error); }); }
async function dbDel(s, k)   { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readwrite"); t.objectStore(s).delete(k); t.oncomplete = () => res(); t.onerror = () => rej(t.error); }); }
async function dbClear(s)    { const db = await initDB(); return new Promise((res, rej) => { const t = db.transaction([s], "readwrite"); t.objectStore(s).clear(); t.oncomplete = () => res(); t.onerror = () => rej(t.error); }); }
...
function updateSEOMeta(apiData) {
  ...
  const query = new URLSearchParams(location.search).get("q") || (location.pathname.startsWith("/search/") ? location.pathname.replace("/search/","").replace(/-/g," ") : "");
  if (!query) return;
  const top = results[0];
  ...
  const jsonLd = { "@context":"https://schema.org", "@type":"WebSite", name:"AniFox", url:location.origin, potentialAction:{"@type":"SearchAction", target:`${location.origin}/?q={search_term_string}`, "query-input":"required name=search_term_string"} };
  ...
  const script = document.createElement("script"); script.type = "application/ld+json"; script.textContent = JSON.stringify(jsonLd); script.setAttribute("data-dynamic", ""); document.head.appendChild(script);
}
...
window.toggleFavorite = async (title, link) => {
  try {
    ...
    if (location.hash === '#favorites' || location.search.includes("page=favorites")) await renderFavoritesPage();
  } catch (e) {
...
window.removeFromHistory = async (e, id) => { e.stopPropagation(); try { await dbDel(STORE_SEARCH_HISTORY, id); renderWeekly(); } catch (e) { console.error("Ошибка удаления из истории:", e); } };
window.clearSearchHistory = async () => { if (confirm("Очистить историю?")) { try { await dbClear(STORE_SEARCH_HISTORY); renderWeekly(); } catch (e) { console.error("Ошибка очистки истории:", e); showNote("Ошибка при очистке истории", "error"); } } };
...
window.clearFavorites = async () => {
  if (confirm("Очистить всё избранное?")) {
    try { await dbClear(STORE_FAVORITES); renderFavoritesPage(); showNote("Избранное очищено", "success"); } catch (e) {
      console.error("Ошибка clearFavorites:", e); showNote("Ошибка при очистке избранного", "error");
    }
  }
};
...
async function search() {
  ...
  try {
    ...
    const seen = new Set();
    const currentSearchResults = (data.results || []).filter(item => { const key = item.title.trim().toLowerCase(); if (seen.has(key)) return false; seen.add(key); return true; });
