...
async function createAnimeCard(item) {
  ...
  // Проверяем, есть ли данные для модального окна информации
  const hasInfoData =
    item.material_data &&
    (item.material_data.description ||
      item.material_data.rating ||
      item.material_data.duration ||
      item.material_data.status ||
      (item.material_data.studios && item.material_data.studios.length > 0) ||
      (item.genres && item.genres.length > 0) ||
      item.year);

  // Проверяем, есть ли данные для кнопки поделиться
  const hasShareData = item.title && item.link;
    ...
    if (modalTitle) {
      ...
      if (link) {
        ...
        modalBtn.setAttribute(
          "onclick",
          `toggleFavorite('${title.replace(/'/g, "\\'")}','${link}')`
        );
      }
    }
  }
...
window.shareAnime = (itemRaw) => {
  ...
  const url = `${location.origin}/search/${toSlug(
...
window.showAnimeInfo = async (itemRaw) => {
  ...
  try {
    ...
    const description =
      extendedInfo.description || md.description || "Описание отсутствует.";
    ...
    const studios = extendedInfo.studios.length
      ? extendedInfo.studios
      : md.studios || [];
      ...
      <div class="modal-content" onclick="event.stopPropagation()">
        ...
        <div class="modal-grid">
          ...
          <div class="modal-right">
            <h2 class="modal-title">${item.title}</h2>
            <p class="modal-orig">${item.title_orig || ""}</p>
            <p class="modal-meta">Год: <b>${item.year || "—"}</b> | Тип: <b>${
      item.type || "—"
    }</b> | Качество: <b>${item.quality || "—"}</b></p>
            <div class="modal-genres">${(item.genres || [])
              .map((g) => `<span class="genre-tag">${g}</span>`)
              .join("")}</div>
            <div class="modal-desc">${
              md.description || "Описание отсутствует."
            }</div>
          </div>
        </div>
      </div>
  ...
  <div class="screenshot-viewer-overlay" onclick="closeScreenshotViewer()">
    <div class="screenshot-viewer-content" onclick="event.stopPropagation()">
      ...
      <div class="screenshot-viewer-image-container">
        ...
        }" class="screenshot-viewer-image" id="screenshotViewerImage">
  ...
  try {
    ...
    for (const fav of list) {
      try {
        ...
        console.error(
          "Error creating card for favorite:",
          fav.title,
          cardError
        );
...
<div class="card fade-in">
  ...
  <iframe class="single-player" src="${
    fav.link
  }" allowfullscreen loading="lazy" title="Плеер: ${fav.title}"></iframe>
  <div class="card-actions">
    <button class="action-btn favorite-btn" data-link="${
      fav.link
    }" onclick="toggleFavorite('${fav.title.replace(/'/g, "\\'")}','${
          fav.link
        }')" title="Удалить из избранного">
      <i class="fas fa-heart"></i>
    </button>
    <!-- Для fallback карточек скрываем кнопки информации и поделиться -->
  </div>
</div>`;
        html += fallbackCard;
      }
